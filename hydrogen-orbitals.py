...
- Cutting planes of the 3D probability density
- 3D contours of the probability density (>80%)
...
import vtk                          # Importing the VTK (Visualization Toolkit), needed to render latex
...
def parse_arguments():
    ...
    parser.add_argument('n', metavar='n', nargs='?', default=4, type=int, help='Principal quantum number n (n >= 1), 6 max.')
...
def prob_dens(n=1, l=0, m=0):
    ...
    # Multiply the absolute value by r if l = 0 since it is a uniform radial distribution (we want see the radial probability in such a case)
    if l == 0:
        dens *= r
...
def plot_slice(orbital, cpos='iso', cut_plane='x', eq=''):
    ...
    pl.add_mesh_slice(
        ...
        generate_triangles = True,          # triangles to fill the cut plane
...
# Execute the main code only if this script is running directly, not when imported as a module
if __name__ == "__main__":
    ...
    pl = pv.Plotter()                           # Initialize the PyVista plotter
    title_bar = '$\\mid\\psi\\mid^2r^2$' if args.l == 0 else '$\\mid\\psi\\mid^2$'  # Define the title for the scalar bar
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    plot_slice(grid, cut_plane='x', eq=expr_prob)   # clip plane at x=0
    plot_slice(grid, cut_plane='y', eq=expr_prob)   # clip plane at y=0
    plot_slice(grid, cut_plane='z', eq=expr_prob)   # clip plane at z=0
    ...
    points = grid.points[indices].copy()                                    # copy the points to handle them
    ...
    pl.add_mesh(
        point_cloud,
        style='points_gaussian',        # points are not sharp
        render_points_as_spheres=False, # points are dots
        point_size=0.7,                 # size of the gaussian dots
        emissive=False,                 # points are just reflective
        show_scalar_bar=False,          # no scale bar is needed
        color=[0, 200, 255]             # cian to better view
    )
